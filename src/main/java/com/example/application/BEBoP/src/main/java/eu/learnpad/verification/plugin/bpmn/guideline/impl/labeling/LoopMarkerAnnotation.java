...
public class LoopMarkerAnnotation extends abstractGuideline {
									...
									for (Artifact artifact : process.getArtifacts()) {
										if (artifact instanceof Association) {
											...
											if (asso.getSourceRef() != null && asso.getTargetRef() != null) {
												...
												boolean flag2 = asso.getSourceRef().equals(a)
														|| asso.getTargetRef().equals(a);
	...
	protected void searchSubProcess(SubProcess sub) {
		...
		if (num > 0) {
			this.Suggestion += Messages.getString("LoopMarkerAnnotation.SuggestionSubprocessKO", l) + sub.getName() //$NON-NLS-1$
					+ " "; //$NON-NLS-1$
			this.status = false;
		}
