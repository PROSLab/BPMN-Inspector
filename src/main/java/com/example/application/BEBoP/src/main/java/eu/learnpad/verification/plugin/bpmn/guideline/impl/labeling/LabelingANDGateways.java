...
public class LabelingANDGateways extends abstractGuideline {
			...
			if (rootElement instanceof Process) {
				...
				for (FlowElement fe : process.getFlowElements()) {
					if (fe instanceof SubProcess) {
						...
						if (!gateway.getIncoming().isEmpty()) {
							for (SequenceFlow sq : gateway.getIncoming()) {
								if (sq.getName() != null) {
									if (sq.getName().length() >= 1) {
										num++;
										elementsBPMN.add(sq);
						...
						if (!gateway.getOutgoing().isEmpty()) {
	...
	protected void searchSubProcess(SubProcess sub) {
		...
		if (num > 0) {
			this.Suggestion += Messages.getString("LabelingANDGateways.SuggestionSubprocessKO", l) + sub.getName() //$NON-NLS-1$
					+ " "; //$NON-NLS-1$
			this.status = false;
		}
