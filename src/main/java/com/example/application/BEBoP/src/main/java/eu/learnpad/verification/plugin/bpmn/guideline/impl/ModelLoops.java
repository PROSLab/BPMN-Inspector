			...
			if (rootElement instanceof Process) {
				...
				for (FlowElement fe : process.getFlowElements()) {
					...
					if (fe instanceof SubProcess) {
						...
						/*
						 * if (fe instanceof Gateway ) { Gateway g =(Gateway) fe; String nameclass =
						 * fe.getClass().getName(); List<SequenceFlow> out = g.getOutgoing(); for
						 * (SequenceFlow sequenceFlow : out) { FlowNode target =
						 * sequenceFlow.getTargetRef(); String nameclasst = target.getClass().getName();
						 * //String targetid = sequenceFlow.getTargetRef().getId();
						 * if(nameclasst.equals(nameclass)){ Gateway t = (Gateway)target;
						 * List<SequenceFlow> in = t.getIncoming(); for (SequenceFlow sequenceFlowin :
						 * in) { FlowNode s = sequenceFlowin.getSourceRef(); if(s.equals(g)){
						 * //flag=true; } } GatewayDirection dir = t.getGatewayDirection();
						 * if(dir.equals(GatewayDirection.DIVERGING)){ flag=true; }
