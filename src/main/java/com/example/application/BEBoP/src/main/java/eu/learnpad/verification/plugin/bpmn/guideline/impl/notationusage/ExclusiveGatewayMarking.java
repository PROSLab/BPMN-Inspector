...
public class ExclusiveGatewayMarking extends abstractGuideline {
	...
	protected void findGL(Definitions diagram) {
		...
		//
		/*
		 * for (BPMNEdge bpmnEdge : BPMNEdges.values()) { BaseElement bpmnelement =
		 * bpmnEdge.getBpmnElement(); if(bpmnelement instanceof SequenceFlow){
		 * bpmnelement.getId(); List<Point> points = bpmnEdge.getWaypoint();
		 * if(points!=null) if(points.size()>2){
		 * 
		 * } } }
		 * 
		 */
