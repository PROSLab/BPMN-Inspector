  ...
  AtmosphereRequest: function (options) {
    ...
    function _handleProtocol(request, message) {
      ...
      if (
        ...
        request.enableProtocol &&
        request.firstMessage &&
        atmosphere.util.browser.msie &&
        +atmosphere.util.browser.version.split(".")[0] < 10
      ) {
    ...
    function _reconnectWithFallbackTransport(errorMessage) {
      ...
      if (
        (_request.reconnect && _request.transport !== "none") ||
        _request.transport == null
      ) {
    ...
    function _attachHeaders(request, url) {
      ...
      if (rq.contentType !== "") {
        ...
        url +=
          "&Content-Type=" +
          (rq.transport === "websocket"
            ? rq.contentType
            : encodeURIComponent(rq.contentType));
      }
      ...
      atmosphere.util.each(rq.headers, function (name, value) {
        var h = atmosphere.util.isFunction(value)
          ? value.call(this, rq, request, _response)
          : value;
        if (h != null) {
    ...
    function _triggerOpen(rq) {
      if (!rq.isOpen) {
        ...
        _response.state === "messageReceived" &&
        (rq.transport === "jsonp" || rq.transport === "long-polling")
      ) {
    ...
    function _executeRequest(request) {
      ...
      if (
        rq.transport === "jsonp" ||
        (rq.enableXDR && atmosphere.util.checkCORSSupport())
      ) {
        _jsonp(rq);
        return;
      }
      ...
      var reconnectFExec = function (force) {
        ...
        if (
          force ||
          (rq.reconnect && _requestCount <= rq.maxReconnectOnClose)
        ) {
      ...
      var reconnectF = function (force) {
        if (atmosphere._beforeUnloadState) {
          ...
          atmosphere.util.debug(
            new Date() +
              " Atmosphere: reconnectF: execution delayed due to _beforeUnloadState flag"
          );
          setTimeout(function () {
      ...
      if (
        rq.force ||
        (rq.reconnect &&
          (rq.maxRequest === -1 || rq.requestCount++ < rq.maxRequest))
      ) {
        rq.force = false;
        ...
        ajaxRequest.onreadystatechange = function () {
          _debug(
            "ajaxRequest.onreadystatechange, new state: " +
          ...
          if (rq.transport !== "polling") {
              ...
              if (!rq.isOpen && _canLog("warn")) {
                atmosphere.util.warn(
                  rq.transport +
                    ...
                    status +
                    " " +
                    (ajaxRequest.statusText || "Unable to connect")
                );
              }
              disconnected();
              return;
            ...
            if (
              (!rq.enableProtocol || !request.firstMessage) &&
              (ajaxRequest.readyState === 2 ||
                (ajaxRequest.readyState > 2 && !rq.isOpen))
            ) {
              // Firefox incorrectly send statechange 0->2 when a reconnect attempt fails. The above checks ensure that onopen is not called for these
              ...
              if (
                atmosphere.util.browser.mozilla &&
                _response.ffTryingReconnect
              ) {
          ...
          if (update) {
            ...
            if (
              rq.transport === "long-polling" &&
              atmosphere.util.trim(responseText).length === 0
            ) {
            ...
            if (rq.transport === "streaming") {
              if (!atmosphere.util.browser.opera) {
                ...
                atmosphere.util.iterate(function () {
                  if (
                    _response.status !== 500 &&
                    ajaxRequest.responseText.length > rq.lastIndex
                  ) {
                    try {
                    ...
                    _response.state = "messageReceived";
                    var message = ajaxRequest.responseText.substring(
                      rq.lastIndex
                    );
            ...
            if (rq.suspend) {
              _response.state =
                _response.status === 0 ? "closed" : "messageReceived";
            } else {
              _response.state = "messagePublished";
            }

            var isAllowedToReconnect =
              !closeStream &&
    ...
    function _doRequest(ajaxRequest, request, create) {
      ...
      if (!_request.dropHeaders) {
        ...
        atmosphere.util.each(request.headers, function (name, value) {
          var h = atmosphere.util.isFunction(value)
            ? value.call(this, ajaxRequest, request, create, _response)
            : value;
          if (h != null) {
    ...
    function _reconnect(ajaxRequest, request, delay) {
      ...
      if (request.reconnect || (request.suspend && _subscribed)) {
        ...
        _response.reason =
          status === 0 ? "Server resumed the connection or down." : "OK";
        ...
        open: function () {
          ...
          stop = atmosphere.util.iterate(function () {
            try {
              ...
              if (
                !cdoc.body ||
                !cdoc.body.firstChild ||
                cdoc.body.firstChild.nodeName.toLowerCase() !== "pre"
              ) {
                // Injects a plaintext element which renders text without interpreting the HTML and cannot be stopped
                // it is deprecated in HTML5, but still works
                var head =
                  cdoc.head ||
                  cdoc.getElementsByTagName("head")[0] ||
    ...
    function _pushWebSocket(message) {
      var msg = atmosphere.util.isBinary(message)
        ? message
      ...
      try {
        ...
        _reconnectWithFallbackTransport(
          "Websocket failed. Downgrading to " +
            _request.fallbackTransport +
            " and resending " +
            message
        );
    ...
    function _f(response, f) {
      switch (response.state) {
        ...
        case "error":
          var dbgReasonPhrase =
            typeof response.reasonPhrase != "undefined"
        ...
        case "closed":
          var closed =
            typeof _request.closed !== "undefined" ? _request.closed : false;
          ...
          if (!closed) {
            ...
            _debug(
              "Request already closed, not firing onClose (" +
                response.state +
                " case)"
            );
          }
    ...
    function _invokeCallback() {
      ...
      var messages =
        isString && _request.trackMessageLength
          ? _response.messages.length > 0
            ? _response.messages
            : [""]
          : new Array(_response.responseBody);
      for (var i = 0; i < messages.length; i++) {
        if (messages.length > 1 && messages[i].length === 0) {
          continue;
        }
        _response.responseBody = isString
          ? atmosphere.util.trim(messages[i])
          : messages[i];
...
atmosphere.subscribe = function (url, callback, request) {
  ...
  uuid =
    typeof request !== "undefined" && typeof request.uuid !== "undefined"
      ? request.uuid
      : 0;
...
atmosphere.util = {
  ...
  param: function (params) {
    ...
    function add(key, value) {
      value = atmosphere.util.isFunction(value)
        ? value()
        : value == null
        ? ""
        : value;
  ...
  checkCORSSupport: function () {
    if (
      ...
      +atmosphere.util.browser.version.split(".")[0] < 11
    ) {
      return true;
    } else if (
      atmosphere.util.browser.opera &&
      +atmosphere.util.browser.version.split(".") < 12.0
    ) {
      return true;
    }
    ...
    // KreaTV 4.1 -> 4.4
      ...
      atmosphere.util.trim(navigator.userAgent).slice(0, 16) ===
      "KreaTVWebKit/531"
    ) {
      return true;
    }
    // KreaTV 3.8
      ...
      atmosphere.util.trim(navigator.userAgent).slice(-7).toLowerCase() ===
      "kreatel"
    ) {
      return true;
    }
...
(function () {
  ...
  atmosphere.util.browser.vmajor =
    atmosphere.util.browser.version.split(".")[0];
  ...
  if (
    atmosphere.util.browser.msie ||
    (atmosphere.util.browser.mozilla &&
      +atmosphere.util.browser.version.split(".")[0] === 1)
  ) {
  ...
  beforeUnload: function () {
    ...
    atmosphere._beforeUnloadTimeoutId = setTimeout(function () {
      atmosphere.util.debug(
        new Date() +
          " Atmosphere: " +
          "beforeunload event timeout reached. Reset _beforeUnloadState flag"
      );
